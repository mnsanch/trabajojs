...
<div class="CircularSTD contenedorlogin separacionprincipal col-10 col-sm-5">
    ...
    <form action="<?=url.'?controller=producto&action=modificar'?>" method="post">
        ...
        <input class="inputlogin" type="text" id="nombre" name="nombre" value="<?=$_POST['nombre']?>" required>

        <label class="separacionsecundaria" for="precio">Precio:</label>
        <input class="inputlogin" type="number" id="precio" name="precio" step="0.01" value="<?=$_POST['precio']?>" required>

        <label class="separacionsecundaria" for="imagen">Ruta de la Imagen:</label>
        <input class="inputlogin" type="text" id="imagen" name="imagen" value="<?=$_POST['imagen']?>" required>

        <label class="separacionsecundaria" for="descripcion">Descripción:</label>
        <textarea class="inputlogin" id="descripcion" name="descripcion" required><?=$_POST['descripcion']?></textarea>
        ...
        if (isset($_POST['alcohol'])) {
            ...
            <div>
                ...
                <input type="radio" id="si" name="alcohol" value="si" required checked>
            </div>
